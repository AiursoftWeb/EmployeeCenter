    ...
    ILogger<ExportService> logger) : IScopedDependency
{
    ...
    public async Task ExportAsync()
    {
        ...
        if (Directory.Exists(_exportRoot))
        {
            ...
            {
                // Preserve legacy GitLab backups, even though this app no longer maintains them.
    ...
    private async Task ExportGlobalSettings()
    {
        ...
        var activeSettingKeys = SettingsMap.Definitions
        ...
        var exportableSettings = settings
            .Where(setting => activeSettingKeys.Contains(setting.Key))
            .Select(setting => new GlobalSetting
            {
                Key = setting.Key,
